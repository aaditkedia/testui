...
// Duotone gradients standing in for AIV event photography.
const PHOTOS = [
...
function PhotoBlock({
  ...
  return (
    <div className={`relative overflow-hidden ${className}`}>
      <div
        className={`absolute inset-0 ${kenburns ? "animate-[kenburns_18s_ease-in-out_infinite_alternate]" : ""}`}
        style={{ background: gradient }}
      />
      {/* grain */}
      <div className="absolute inset-0 opacity-[0.13] mix-blend-overlay" style={{ backgroundImage: GRAIN }} />
      {/* vignette */}
      <div className="absolute inset-0" style={{ boxShadow: "inset 0 0 160px 30px rgba(0,0,0,0.6)" }} />
      {label && (
  ...
  return (
    <div className="bg-[#0A0A0B] font-body text-[#FAFAF7]">
      ...
      <section className="relative flex min-h-screen items-end overflow-hidden">
        <div className="absolute inset-0 animate-[kenburns_22s_ease-in-out_infinite_alternate]" style={{ background: "radial-gradient(120% 90% at 70% 20%,#13384a,#0a0a0b 70%)" }} />
      ...
      <section className="mx-auto max-w-[1200px] px-6 py-28 sm:px-10">
        <div className="grid gap-12 lg:grid-cols-[1.5fr_1fr] lg:gap-[120px]">
          <PhotoBlock gradient={PHOTOS[0]} label="AIV demo day" kenburns className="aspect-[4/5] w-full rounded-sm" />
      ...
      <section className="relative flex min-h-screen items-center overflow-hidden">
        <PhotoBlock gradient={PHOTOS[1]} kenburns className="absolute inset-0" />
      ...
      <section>
        {a.perks.map((p, i) => (
          <div key={p.title} className="relative flex min-h-[80vh] items-center overflow-hidden">
            <PhotoBlock gradient={PHOTOS[i % PHOTOS.length]} kenburns className="absolute inset-0" />
            <motion.div
              initial={{ opacity: 0, y: 30 }}
          ...
          <div className="mt-16 grid grid-cols-2 gap-3 sm:grid-cols-4">
            {["Demo Day · SF", "Founder Dinner · NYC", "Summit · Lisbon", "Hack Night · Bangalore"].map((ev, i) => (
              <div key={ev} className="group relative">
                <PhotoBlock gradient={PHOTOS[i % PHOTOS.length]} className="aspect-video w-full rounded-sm" />
